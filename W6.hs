...
split s
  | length for == 0 = Nothing
  | length sur <= 1 = Nothing
  | otherwise       = Just (for, (tail sur))
  where (for, sur) = break (== ' ') s
...
checkDuplicate (for,sur) = if for /= sur then Just (for, sur) else Nothing
...
myDrop mi ml = do i <- mi
                  l <- ml
                  if i < 0 || i > (length l)
                    then Nothing
                    else return $ drop i l
------------------------------------------------------------------------------
...
binom n k = if n == 0 && k > 0
            then msg ("B(0," ++ show k ++ ")") >> return 0
            else do i <- binom (n-1) (k-1)
                    j <- binom (n-1) k
                    msg ("B(" ++ show n ++ "," ++ show k ++ ")")
                    return $ i + j
...
lengthAndSum (x:xs) = do len <- lengthAndSum xs
                         modify (+x)
                         return $ 1 + len
...
oddUpdate x = do xs <- get
                 if elem x xs
                 then put (delete x xs)
                 else put (x:xs)
...
mapM2 _ _ [] = return []
mapM2 _ [] _ = return []
mapM2 op (x:xs) (y:ys) = do result <- op x y
