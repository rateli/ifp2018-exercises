...
funny :: [String] -> String
funny strings = map toUpper $ intercalate " " filtered
  where filtered = filter (\x -> length x > 5) strings
...
powers :: Int -> Int -> [Int]
powers n max = takeWhile (<= max) [n^exp | exp <- [0..]]
...
mymaximum cmp def (x:y:xs) = if cmp x y == GT then mymaximum cmp def (x:xs)
                             else mymaximum cmp def (y:xs)
...
interpreter' [] _ _ = []
interpreter' (cmd:commands) x y
  | cmd == "up"     = interpreter' commands x (y+1)
  | cmd == "down"   = interpreter' commands x (y-1)
  | cmd == "right"  = interpreter' commands (x+1) y
  | cmd == "left"   = interpreter' commands (x-1) y
  | cmd == "printX" = show x : interpreter' commands x y
  | cmd == "printY" = show y : interpreter' commands x y
  | otherwise       = []
