...
addEithers (Right _) (Left str) = Left str
...
measure Leaf = Leaf
measure (Node n l r) = Node subTreeSize (measure l) (measure r)
                       where subTreeSize = treeSize (Node n l r)
...
rgb Green = [0, 1, 0]
rgb Blue  = [0, 0, 1]
rgb (Darken a b) = map (\i -> i * (1-a)) (rgb b)
rgb (Mix a b) = zipWith (\i j -> min (i+j) 1) (rgb a) (rgb b)
